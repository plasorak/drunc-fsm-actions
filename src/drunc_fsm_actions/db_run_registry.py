...
class DBRunRegistry(FSMAction):
    def __init__(self, configuration, _dry_run=False) -> None:
        ...
        try:
            ...
            msg = (
                ...
                "'run_registry_configuration' section, or the entire"
                "'run_registry_configuration' section"
            )
    ...
    def pre_start(self, _input_data: dict, _context, **kwargs):
        ...
        # Seems like run_number isn't in _input_data
        # in post_drain_dataflow so need to initialise it here
    ...
    def post_drain_dataflow(self, _input_data, _context, **kwargs) -> None:
        if not self.dry_run:
            try:
                requests.get(
                    self.API_SOCKET
